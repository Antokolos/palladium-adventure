...
func _ready():
	...
	match (__PLDRT.settings.tablet_orientation):
		PLDSettings.TABLET_VERTICAL:
			tablet_orientation.select(0)
		PLDSettings.TABLET_HORIZONTAL:
			tablet_orientation.select(1)
	_on_TabletOrientation_item_selected(__PLDRT.settings.tablet_orientation)

	joypad_type.add_item("XBOX", PLDSettings.JOYPAD_XBOX)
	joypad_type.add_item("PlayStation", PLDSettings.JOYPAD_PS)
	joypad_type.add_item("NINTENDO", PLDSettings.JOYPAD_NINTENDO)
	match (__PLDRT.settings.joypad_type):
	...
	_on_JoypadType_item_selected(__PLDRT.settings.joypad_type)

	vsync.pressed = __PLDRT.settings.vsync
	_on_Vsync_pressed()

	fullscreen.pressed = __PLDRT.settings.fullscreen
	...
	pause_on_joypad_disconnected.pressed = __PLDRT.settings.pause_on_joy_disconnected
	_on_PauseOnJoypadDisconnected_pressed()

	disable_mouse_if_joypad_connected.pressed = __PLDRT.settings.disable_mouse_if_joy_connected
	...
	match (__PLDRT.settings.difficulty):
		...
		PLDSettings.DIFFICULTY_HARD:
			difficulty.select(1)
	# _on_Difficulty_item_selected(__PLDRT.settings.difficulty) -- not needed here, will be done on player _ready

	var i = 0
	var ssize = OS.get_screen_size()
	for r in __PLDRT.settings.resolutions:
		resolution.add_item("%d x %d" % [ssize.x * __PLDRT.settings.resolutions[i].height / ssize.y, __PLDRT.settings.resolutions[i].height], i)
		i = i + 1
	resolution.add_item("Native (%d x %d)" % [ssize.x, ssize.y], i)
	match (__PLDRT.settings.resolution):
		PLDSettings.RESOLUTION_480:
			resolution.select(0)
		PLDSettings.RESOLUTION_576:
			resolution.select(1)
		PLDSettings.RESOLUTION_720:
			resolution.select(2)
		PLDSettings.RESOLUTION_1080:
			resolution.select(3)
		_:
			resolution.select(i)
	...
	match (__PLDRT.settings.aa_quality):
		PLDSettings.AA_DISABLED:
			aa.select(0)
		PLDSettings.AA_2X:
			aa.select(1)
		PLDSettings.AA_4X:
	...
	master_volume_node.value = __PLDRT.settings.master_volume
	music_volume_node.value = __PLDRT.settings.music_volume
	sound_volume_node.value = __PLDRT.settings.sound_volume
	speech_volume_node.value = __PLDRT.settings.speech_volume
	
	use_image_adjust.pressed = __PLDRT.settings.use_image_adjust
	brightness.value = __PLDRT.settings.brightness
	contrast.value = __PLDRT.settings.contrast
	saturation.value = __PLDRT.settings.saturation
...
func simulate_esc():
	...
	__PLDRT.common_utils.toggle_pause_menu()
	__PLDRT.common_utils.show_mouse_cursor_if_needed_in_game(hud)

func save_to_slot(slot):
	__PLDRT.game_state.save_state(slot)
	simulate_esc()

func load_from_slot(slot):
	__PLDRT.game_state.initiate_load(slot)
	simulate_esc()
...
func _on_TabletOrientation_item_selected(ID):
	if ID == PLDSettings.TABLET_HORIZONTAL:
	...
	tablet_panel.update()
	home_button.update()
	__PLDRT.settings.tablet_orientation = ID

func _on_JoypadType_item_selected(ID):
	__PLDRT.settings.joypad_type = ID

func _on_Vsync_pressed():
	var vs = vsync.is_pressed() if vsync else __PLDRT.settings.vsync
	__PLDRT.settings.set_vsync(vs)

func _on_Fullscreen_pressed():
	var fs = fullscreen.is_pressed() if fullscreen else __PLDRT.settings.fullscreen
	__PLDRT.settings.set_fullscreen(fs)

func _on_InvertYAxis_pressed():
	var enabled = invert_yaxis.is_pressed() if invert_yaxis else __PLDRT.settings.invert_yaxis
	__PLDRT.settings.set_invert_yaxis(enabled)
...
func _on_CutoffEnabled_pressed():
	...
	__PLDRT.settings.set_cutoff_enabled(ce)

func _on_PauseOnJoypadDisconnected_pressed():
	var pjd = pause_on_joypad_disconnected.is_pressed() if pause_on_joypad_disconnected else __PLDRT.settings.pause_on_joy_disconnected
	__PLDRT.settings.set_pause_on_joy_disconnected(pjd)

func _on_DisableMouseIfJoypadConnected_pressed():
	var mjc = disable_mouse_if_joypad_connected.is_pressed() if disable_mouse_if_joypad_connected else __PLDRT.settings.disable_mouse_if_joy_connected
	__PLDRT.settings.set_disable_mouse_if_joy_connected(mjc)

func _on_Quality_item_selected(ID):
	__PLDRT.settings.set_quality(ID)

func _on_Difficulty_item_selected(ID):
	__PLDRT.settings.set_difficulty(ID)

func _on_Resolution_item_selected(ID):
	__PLDRT.settings.set_resolution(ID)
...
func _on_Subtitles_pressed():
	...
	__PLDRT.settings.set_subtitles(s)

func _on_MasterVolume_value_changed(value):
	__PLDRT.settings.set_master_volume(value)

func _on_MusicVolume_value_changed(value):
	__PLDRT.settings.set_music_volume(value)

func _on_SoundVolume_value_changed(value):
	__PLDRT.settings.set_sound_volume(value)

func _on_SpeechVolume_value_changed(value):
	__PLDRT.settings.set_speech_volume(value)

func _on_UseImageAdjust_pressed():
	var ia = use_image_adjust.is_pressed() if use_image_adjust else __PLDRT.settings.use_image_adjust
	__PLDRT.settings.set_use_image_adjust(ia)
	if brightness:
