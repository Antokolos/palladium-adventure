...
func _ready():
	...
	__PLDRT.cutscene_manager.connect("camera_borrowed", self, "_on_camera_borrowed")
	__PLDRT.cutscene_manager.connect("camera_restored", self, "_on_camera_restored")
	__PLDRT.common_utils.show_mouse_cursor_if_needed_in_game(self)
	__PLDRT.game_state.emit_stats_signals_for_everyone()
	synchronize_items()
	select_active_item()
...
func queue_popup_message(template, args = [], fade = false, timeout_max = MESSAGE_TIMEOUT_MAX_S):
	if not template:
		return
	if __PLDRT.game_state.get_message_state(template):
		__PLDRT.game_state.set_message_state(template, false)
...
func process_popup_messages(delta):
	if popup_message_queue.empty():
		return
	var is_in_conversation = __PLDRT.conversation_manager.conversation_is_in_progress()
...
func is_in_conversation():
	return conversation.visible or __PLDRT.conversation_manager.conversation_is_in_progress()
...
func _on_shader_cache_processed():
	if cutscene_mode:
		return
	var wasd = __PLDRT.common_utils.get_input_control("movement_forward", false) \
...
func _on_item_taken(item_id, count_total, count_taken, item_path):
	queue_popup_message(
		...
		[ tr(PLDDB.get_item_name(item_id)) ] if count_taken == 1 else [ tr(PLDDB.get_item_name(item_id)), count_taken ],
		true,
...
func _on_item_removed(item_id, count_total, count_removed):
	if count_total == 0 and PLDDB.is_item_stackable(item_id):
		queue_popup_message(
			"MESSAGE_ITEM_REMOVED",
			[ tr(PLDDB.get_item_name(item_id)) ],
			true,
			MESSAGE_TIMEOUT_ITEM_S
		)
...
func _on_preview_opened(item):
	...
	label_close_node.text = (
		...
		+ " | "
		+ __PLDRT.common_utils.get_input_control("item_preview_zoom_in", true, true)
		+ tr("ACTION_ZOOM_IN")
		+ " | "
		+ __PLDRT.common_utils.get_input_control("item_preview_zoom_out", true, true)
		+ tr("ACTION_ZOOM_OUT")
	)
...
func _on_preview_closed(item):
	...
	actions_panel.hide()
	show_game_ui(true)
	if __PLDRT.game_state.get_quick_items_count() > 1:
		queue_popup_message("MESSAGE_CONTROLS_ITEMS", [__PLDRT.common_utils.get_input_control("active_item_next", false), __PLDRT.common_utils.get_input_control("active_item_back", false)])
		if not __PLDRT.common_utils.has_joypads():
			queue_popup_message("MESSAGE_CONTROLS_ITEMS_KEYS", [__PLDRT.common_utils.get_input_control("active_item_1", false), __PLDRT.common_utils.get_input_control("active_item_6", false)])
	select_active_quick_item() # This will also call camera.activate_item_use() if needed
...
func select_active_item():
	...
	var idx = 0
	for item in items:
		if items[idx].item_id and items[idx].item_id != PLDDB.TakableIds.NONE:
			...
			if idx == active_item_idx:
				...
				if inventory.is_visible_in_tree():
					info_label.text = __PLDRT.common_utils.get_input_control("active_item_toggle") + tr("ACTION_TOGGLE_QUICK_ITEM")
			else:
...
func select_active_quick_item():
	...
	for item in items:
		...
		if is_active and (not inventory.is_visible_in_tree() or not is_valid_index(active_item_idx)):
			info_label.text = tr(PLDDB.get_item_name(items[idx].item_id)) if items[idx].item_id and items[idx].item_id != PLDDB.TakableIds.NONE else ""
		idx = idx + 1
...
func _input(event):
	if __PLDRT.game_state.is_video_cutscene():
		return
	...
	if inventory.visible:
		if event.is_action_pressed("inventory_toggle"):
			...
			if item:
				__PLDRT.game_state.set_quick_item(active_quick_item_idx, item.item_id)
				synchronize_items()
		...
		if event.is_action_pressed("inventory_toggle"):
			...
			__PLDRT.game_state.get_hud().queue_popup_message("MESSAGE_CONTROLS_TOGGLE_ITEM_1", [__PLDRT.common_utils.get_input_control("active_item_toggle", false)])
