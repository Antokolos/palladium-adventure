...
func _notification(what):
	match what:
		NOTIFICATION_POST_POPUP:
			#__PLDRT.common_utils.set_mouse_mode(Input.MOUSE_MODE_VISIBLE) #TODO: check it is neeeded
			...
			match input_type:
				...
				PLDSettings.InputType.JOYPAD_BUTTON:
					if event is InputEventJoypadButton:
						captured_event = event
						label_result.text = __PLDRT.common_utils.joy_button_to_string(event.get_button_index())
						enable_buttons()
				PLDSettings.InputType.JOYPAD_MOTION:
					if event is InputEventJoypadMotion:
						...
						if v > AXIS_VALUE_THRESHOLD or v < -AXIS_VALUE_THRESHOLD:
							captured_event = event
							label_result.text = __PLDRT.common_utils.joy_axis_to_string(a, v)
							enable_buttons()
				PLDSettings.InputType.MOUSE_BUTTON:
					if event is InputEventMouseButton:
						captured_event = event
						label_result.text = __PLDRT.common_utils.mouse_button_to_string(event.get_button_index())
						enable_buttons()

func _on_ApplyButton_pressed():
	if captured_event:
		for input_type in input_types:
			__PLDRT.settings.erase_action_event_of_type(action_name, input_type)
