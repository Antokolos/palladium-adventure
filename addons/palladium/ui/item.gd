...
func set_appearance(is_quick_item, selected):
	...
	if is_quick_item and __PLDRT.settings.hide_quick_item_key_labels:
		label_key.hide()
	else:
		label_key.show()
	if is_quick_item:
...
func set_item_data(item_id, item_count):
	if not __PLDRT.game_state.is_item_registered(item_id):
		push_error("Unknown item_id in set_item_data: " + str(item_id))
		return
	var item_data = __PLDRT.game_state.get_registered_item_data(item_id)
	...
	get_node("ItemBox/LabelDesc").text = tr(PLDDB.get_item_name(item_id))
...
func is_empty():
	return not item_id or item_id == PLDDB.TakableIds.NONE

func is_weapon():
	return (
		PLDDB.is_weapon_stun(item_id)
		or PLDDB.is_weapon_ranged(item_id)
	)

func can_be_given():
	if is_empty():
		return false
	return PLDDB.ITEMS.has(item_id) and PLDDB.ITEMS[item_id].can_give
...
func get_model_instance():
	...
	return set_portals_global_flag(instance.instance()) if instance else null

func get_model_use_instance():
	var instance = load(model_use_path) if model_use_path else null
	return set_portals_global_flag(instance.instance()) if instance else null

func set_portals_global_flag(instance):
	if instance is MeshInstance:
		instance.portal_mode = MeshInstance.PORTAL_MODE_GLOBAL
		instance.include_in_bound = false
	for m in instance.get_children():
		set_portals_global_flag(m)
	return instance
...
func remove(all_items = false):
	__PLDRT.game_state.remove(item_id, item_count if all_items else 1)
