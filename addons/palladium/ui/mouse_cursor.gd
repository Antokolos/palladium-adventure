...
func _ready():
	...
	__PLDRT.common_utils.connect("mouse_mode_changed", self, "_on_mouse_mode_changed")
	warp_mouse(viewport.get_mouse_position())

func _on_joy_connection_changed(device_id, is_connected):
	if not is_connected:
		if not __PLDRT.settings.pause_on_joy_disconnected:
			return
...
func get_mouse_relative_and_warp_in_center():
