...
func set_joypad_text(control, act):
	if act is InputEventJoypadButton:
		...
		control.get_node("Label").set_text(__PLDRT.common_utils.joy_button_to_string(button_index))
	if act is InputEventJoypadMotion:
		var axis = act.get_axis()
		var axis_value = act.get_axis_value()
		control.get_node("Label").set_text(__PLDRT.common_utils.joy_axis_to_string(axis, axis_value))

func set_mouse_button_text(control, act):
	if act is InputEventMouseButton:
		var button_index = act.get_button_index()
		control.get_node("Label").set_text(__PLDRT.common_utils.mouse_button_to_string(button_index))
