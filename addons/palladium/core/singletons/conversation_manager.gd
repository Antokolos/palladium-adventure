...
func enable_conversation(conversation_name, enable):
	...
	_pldrt.story_node.enable_conversation(cp_story, enable)
...
func stop_conversation(player):
	...
	_pldrt.story_node.increase_visit_count(last_result)
...
func check_story_not_finished(conversation_name, target_name_hint = null):
	...
	return _pldrt.story_node.check_story_not_finished(cp_story)

func check_story_result_was_achieved(conversation_name, result = 0, target_name_hint = null):
	var cp_story = get_story_path(conversation_name, target_name_hint)
	return _pldrt.story_node.check_story_result_was_achieved(cp_story, result)

func init_story(conversation_name, target_name_hint = null, repeatable = false):
	var cp_story = get_story_path(conversation_name, target_name_hint)
	_pldrt.story_node.load_story(cp_story, false, repeatable)
	_pldrt.story_node.init_variables()
	return _pldrt.story_node
...
func start_conversation(player, conversation_name, target = null, initiator = null, is_cutscene = false, cutscene_node = null, repeatable = false, area_to_check : Area = null):
	...
	if _pldrt.story_node.is_disabled(cp_story):
		return
	for actor in _pldrt.story_node.get_actors_name_hints(cp_story):
		var character = _pldrt.game_state.get_character(actor)
		if character:
...
func get_vvalue(dict):
	return dict["ru"] if _pldrt.settings.vlanguage == PLDSettings.VLANGUAGE_RU else (dict["en"] if _pldrt.settings.vlanguage == PLDSettings.VLANGUAGE_EN else null)

func story_choose(player, idx):
	var has_sound = false
	var conversation = _pldrt.game_state.get_hud().conversation
	var conversation_text = conversation.get_node("VBox/VBoxText/HBoxText/ConversationText")
	var conversation_actor = conversation.get_node("VBox/VBoxText/HBoxText/ActorName")
	_pldrt.lipsync_manager.stop_sound_and_lipsync(false)  # If the character has not finished speaking, but the player already decided to continue dialogue
	if is_finalizing or (not _pldrt.story_node.can_choose() and not _pldrt.story_node.can_continue() and idx == 0):
		stop_conversation(player)
	elif _pldrt.story_node.can_choose() and max_choice > 0 and idx < max_choice:
		...
		if _pldrt.story_node.can_continue():
			...
			if vtags and vtags.has("voiceover"):
				...
				var character = _pldrt.game_state.get_character(current_actor_name)
				...
				has_sound = _pldrt.lipsync_manager.play_sound_and_start_lipsync(character, conversation_name, target.name_hint if target else null, vtags["voiceover"], text, transcription, pre_delay, post_delay)
			change_stretch_ratio(conversation)
		conversation.visible = _pldrt.settings.need_subtitles() or not has_sound
		if not has_sound:
			story_proceed(player)
...
func proceed_story_immediately(player):
	if _pldrt.lipsync_manager.is_speaking():
		_pldrt.lipsync_manager.stop_sound_and_lipsync()
	else:
		story_proceed(player)
...
func story_proceed(player):
	...
	if _pldrt.story_node.can_continue():
		...
		if has_voiceover:
			...
			var pre_delay = float(tags["pre_delay"]) if tags and tags.has("pre_delay") else 0.0
			var post_delay = float(tags["post_delay"]) if tags and tags.has("post_delay") else 0.0
			_pldrt.lipsync_manager.play_sound_and_start_lipsync(character, conversation_name, target.name_hint if target else null, vtags["voiceover"], text, transcription, pre_delay, post_delay)
		change_stretch_ratio(conversation)
...
func display_choices(conversation, choices, can_choose):
	...
	for i in range(0, max_choice):
		var cns = str(i + 1)
		var ic = _pldrt.common_utils.get_input_control("dialogue_option_%s" % cns, false) if can_choose else _pldrt.common_utils.get_input_control("dialogue_next", false)
...
func get_current_actor():
	return _pldrt.game_state.get_character(current_actor_name) if current_actor_name else null

func get_previous_actor():
	return _pldrt.game_state.get_character(previous_actor_name) if previous_actor_name else null
