...
func get_conversation_sound_path(conversation_name, target_name_hint = null):
	var locale = "ru" if _pldrt.settings.vlanguage == PLDSettings.VLANGUAGE_RU else ("en" if _pldrt.settings.vlanguage == PLDSettings.VLANGUAGE_EN else null)
	if not locale:
		return null
...
func play_sound_and_start_lipsync(character, conversation_name, target_name_hint, file_name, text = null, transcription = null, pre_delay_time_s = 0.0, post_delay_time_s = 0.0):
	...
	if not stream:
		return false
	if not _pldrt.common_utils.set_stream_loop(stream, false):
		return false
	stop_sound_and_lipsync(false)
...
func continue_conversation():
	...
	if _pldrt.conversation_manager.is_finalizing():
		_pldrt.conversation_manager.stop_conversation(player)
