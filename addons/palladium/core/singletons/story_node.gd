...
func bind_external_functions(story):
	story.bind_external_function("is_room_enabled", _pldrt.game_state, "_is_room_enabled")
	story.bind_external_function("player_is_in_room", _pldrt.game_state, "_player_is_in_room")
	story.bind_external_function("is_untouched", _pldrt.game_state, "_is_untouched")
	story.bind_external_function("is_activated", _pldrt.game_state, "_is_activated")
	story.bind_external_function("is_deactivated", _pldrt.game_state, "_is_deactivated")
	story.bind_external_function("is_paused", _pldrt.game_state, "_is_paused")
	story.bind_external_function("is_final_destination", _pldrt.game_state, "_is_final_destination")
	story.bind_external_function("is_actual", _pldrt.game_state, "_is_actual")
	story.bind_external_function("conversation_is_not_finished", _pldrt.conversation_manager, "conversation_is_not_finished")
	story.bind_external_function("meeting_is_finished", _pldrt.conversation_manager, "meeting_is_finished")
	story.bind_external_function("meeting_is_finished_exact", _pldrt.conversation_manager, "meeting_is_finished_exact")
	story.bind_external_function("meeting_is_not_finished", _pldrt.conversation_manager, "meeting_is_not_finished")
...
func _observe_variable(variable_name, new_value) -> void:
	_pldrt.game_state.story_vars[variable_name] = new_value
...
func init_variables() -> void:
	var storyVars : Dictionary = _pldrt.game_state.story_vars
		...
		if _inkStory.has(locale):
			...
			for party_key in party_keys:
				...
				if story.variables_state.global_variable_exists_with_name(story_key):
					story.variables_state.set(story_key, _pldrt.game_state.is_in_party(party_key))
				...
				if story.variables_state.global_variable_exists_with_name(relationship_key):
					story.variables_state.set(relationship_key, _pldrt.game_state.get_character(party_key).get_relationship())
				...
				if story.variables_state.global_variable_exists_with_name(morale_key):
					story.variables_state.set(morale_key, _pldrt.game_state.get_character(party_key).get_morale())
...
func save_all(slot : int) -> void:
	for locale in AvailableLocales:
		if _currentSessionStories.has(locale):
		...
		saveFile.store_string(
			to_json(_inkStoriesStates[locale])
				if _inkStoriesStates.has(locale)
				else "{}"
		)
		saveFile.close()
	var slotCaptionFile : File = File.new()
