...
var show_path = false
var shader_cache_enabled = false
var vsync = true
var fullscreen = true
...
func get_master_bus_id():
	return AudioServer.get_bus_index("Master")

func set_master_volume(level):
	master_volume = level
	set_volume(get_master_bus_id(), level)

func get_music_bus_id():
	return AudioServer.get_bus_index("Music")

func set_music_volume(level):
	music_volume = level
	set_volume(get_music_bus_id(), level)

func get_sound_bus_id():
	return AudioServer.get_bus_index("Sound")

func set_sound_volume(level):
	sound_volume = level
	set_volume(get_sound_bus_id(), level)

func get_speech_bus_id():
	return AudioServer.get_bus_index("Speech")

func set_speech_volume(level):
	speech_volume = level
	set_volume(get_speech_bus_id(), level)
...
func set_reverb(enable):
	AudioServer.set_bus_effect_enabled(get_sound_bus_id(), 0, enable)
	AudioServer.set_bus_effect_enabled(get_speech_bus_id(), 0, enable)
