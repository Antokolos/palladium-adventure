...
func cannot_use_action(player_node, item):
	if not __PLDRT.settings.is_difficulty_hard():
		return true
	__PLDRT.MEDIA.play_sound(PLDDBMedia.SoundId.MALE_SCREAM_SHORT)
	__PLDRT.game_state.set_surge(__PLDRT.game_state.get_player(), true)
	__PLDRT.game_state.kill_party()
	return true
...
func get_usage_code(player_node):
	...
	if hud and hud.get_active_item():
		...
		if is_empty():
			for id in __PLDRT.DB.get_pedestal_applicable_items():
