...
func _ready():
	for name_hint in __PLDRT.CHARS.get_all_name_hints():
	...
	if Engine.editor_hint:
		return
	__PLDRT.conversation_manager.connect("conversation_finished", self, "_on_conversation_finished")
	...
	__PLDRT.game_state.register_conversation_area(self)
...
func _on_conversation_area_body_entered(body):
	if Engine.editor_hint:
		return false
	if not body.is_in_group("party") or __PLDRT.game_state.is_loading():
		return false
	var player_name_hint = body.get_name_hint()
	for name_hint in conversations.keys():
		if conversations[name_hint].empty() \
			or not __PLDRT.game_state.is_in_party(name_hint) \
		...
		for i in range(conversations_for_name.size()):
			...
			if __PLDRT.conversation_manager.conversation_is_in_progress(conversation) \
				or __PLDRT.conversation_manager.conversation_is_finished(conversation):
				continue
			...
			if conversaton_delay_timer \
				...
				if exclusive:
					__PLDRT.conversation_manager.discard_all_conversations(__PLDRT.game_state.get_player())
...
func _on_ConversationDelayTimer_timeout():
	if not scheduled_conversation:
		return
	if cutscene_node:
		__PLDRT.conversation_manager.start_area_cutscene(scheduled_conversation, cutscene_node, repeatable, exclusive, self if area_dependent else null)
	else:
		__PLDRT.conversation_manager.start_area_conversation(scheduled_conversation, repeatable, exclusive, self if area_dependent else null)
	scheduled_conversation = null
