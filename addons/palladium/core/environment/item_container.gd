...
func is_opened():
	var cs = __PLDRT.game_state.get_container_state(get_path())
...
func open(is_restoring = false, speed_scale = 1.0):
	...
	if not is_restoring:
		__PLDRT.game_state.set_container_state(get_path(), true)
...
func close(is_restoring = false, speed_scale = 1.0):
	...
	if not is_restoring:
		__PLDRT.game_state.set_container_state(get_path(), false)
...
func restore_state():
	var state = __PLDRT.game_state.get_container_state(get_path())
