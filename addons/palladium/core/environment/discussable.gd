...
func get_action_key(pl):
	...
	var c = __PLDRT.game_state.get_companion()
...
func is_conversation_finished_or_not_applicable(player_node):
	...
	for name_hint in conversations.keys():
		...
		for i in range(conversations_for_name.size()):
			...
			return __PLDRT.conversation_manager.conversation_is_finished(conversation)
	return true
...
func use(player_node, camera_node):
	...
	for name_hint in conversations.keys():
		if conversations[name_hint].empty() or not __PLDRT.game_state.is_in_party(name_hint):
			continue
		var conversations_for_name = conversations[name_hint].split(CONVERSATIONS_DELIMITER)
		for i in range(conversations_for_name.size()):
			if not is_conversation_enabled(player_node, name_hint, i):
			...
			var conversation = conversations_for_name[i]
			if cutscene_node:
				__PLDRT.conversation_manager.start_area_cutscene(conversation, cutscene_node, repeatable)
			else:
				__PLDRT.conversation_manager.start_area_conversation(conversation, repeatable)
			return
