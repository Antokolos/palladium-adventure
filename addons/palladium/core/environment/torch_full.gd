...
export(PLDDB.LightIds) var light_id = PLDDB.LightIds.NONE
...
func is_active():
	var ls = __PLDRT.game_state.get_light_state(get_path())
	return (ls == __PLDRT.game_state.LightState.DEFAULT and initially_active) or (ls == __PLDRT.game_state.LightState.ON)
...
func enable(active, update):
	...
	if update:
		__PLDRT.game_state.set_light_state(get_path(), active)
	emit_signal("state_changed", self, active, update)

func use(player_node, camera_node):
	var active = not is_active()
	if active:
		__PLDRT.MEDIA.play_sound(PLDDBMedia.SoundId.FIRE_LIGHTER)
		sound_burning.play()
	else:
		__PLDRT.MEDIA.play_sound(PLDDBMedia.SoundId.FIRE_EXTINGUISH)
		sound_burning.stop()
	enable(active, true)
