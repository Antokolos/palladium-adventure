...
func is_in_speak_mode():
	return __PLDRT.conversation_manager.conversation_is_in_progress()
...
func set_meshes_portal_mode(root, portal_mode, layer_enable):
	for m in root.get_children():
		if m is MeshInstance:
			m.set_portal_mode(portal_mode)
			m.set_layer_mask_bit(5, layer_enable)
		else:
			set_meshes_portal_mode(m, portal_mode, layer_enable)
...
func normalize_angle(look_angle_deg):
	return (
		look_angle_deg
		if abs(look_angle_deg) < ROTATE_HEAD_ANGLE_MAX_DEG
		else (ROTATE_HEAD_ANGLE_MAX_DEG if look_angle_deg > 0 else -ROTATE_HEAD_ANGLE_MAX_DEG)
	)
...
func rotate_head(look_angle_deg):
	...
	if la < look_angle_tgt_deg - ROTATE_HEAD_ANGLE_TOLERANCE_DEG \
		...
		or la <= -ROTATE_HEAD_ANGLE_MAX_DEG \
		or la >= ROTATE_HEAD_ANGLE_MAX_DEG:
...
func process_head_rotation():
	if look_angle_tgt_deg < look_angle_cur_deg - ROTATE_HEAD_ANGLE_TOLERANCE_DEG:
		...
		look_angle_cur_deg = look_angle_cur_deg + ROTATE_HEAD_ANGLE_TOLERANCE_DEG
	elif look_angle_tgt_deg <= -ROTATE_HEAD_ANGLE_MAX_DEG \
		and look_angle_cur_deg > -ROTATE_HEAD_ANGLE_MAX_DEG:
		look_angle_cur_deg = -ROTATE_HEAD_ANGLE_MAX_DEG
	elif look_angle_tgt_deg >= ROTATE_HEAD_ANGLE_MAX_DEG \
		and look_angle_cur_deg < ROTATE_HEAD_ANGLE_MAX_DEG:
		look_angle_cur_deg = ROTATE_HEAD_ANGLE_MAX_DEG
	else:
		return
	if look_angle_cur_deg > 0:
		animation_tree.set("parameters/Blend2_Aux/blend_amount", 0.0)
		animation_tree.set("parameters/Blend2_Head/blend_amount", look_angle_cur_deg / ROTATE_HEAD_ANGLE_MAX_DEG)
	else:
		animation_tree.set("parameters/Blend2_Aux/blend_amount", -look_angle_cur_deg / ROTATE_HEAD_ANGLE_MAX_DEG)
		animation_tree.set("parameters/Blend2_Head/blend_amount", 0.0)
...
func set_loop(anim_name):
	...
	var anim_player = animation_tree.get_node(path) if path and animation_tree.has_node(path) else null
	if anim_player:
		var anim = anim_player.get_animation(anim_name)
		if anim:
			anim.set_loop(true)
		else:
			push_error("Animation %s is not found for path %s" % [anim_name, path])
	else:
		push_error("Animation %s is not found for path %s" % [anim_name, path])
